...
"""
Script to downsample the dataset: hypoxia.dataset
Location: /mnt/nfs/CX000008_DS1/projects/btanasa/hypoxia_newobj_jan2026/SCANVI_INTEGRATED_FILTERED.geneformer_compatible.dataset

"""

)

# reuse the old script : geneformer_hypoxia_downsample_sep10_down_subsampling.oct20.34K.34K.34K.py
...
WORKDIR = Path("/mnt/nfs/CX000008_DS1/projects/btanasa/hypoxia_newobj_jan2026/SCANVI_INTEGRATED_FILTERED.geneformer_compatible.dataset")
workdir = WORKDIR
...
# Combine paths
hypoxia_path = os.path.join(workdir, 'hypoxia.dataset')

# List files in directory
print("\n📄 Files in directory:")
for file in os.listdir(hypoxia_path):
    if os.path.isfile(os.path.join(hypoxia_path, file)):
        print(f"  - {file}")
...
cols_to_check = ["post_filter_annots", "condition"]
...
df_orig = pd.DataFrame({"post_filter_annots": orig_ds["post_filter_annots"], "condition": orig_ds["condition"]})

# Counts: rows = class, cols = condition
counts = pd.crosstab(df_orig["post_filter_annots"], df_orig["condition"]).sort_index()
...
3. Balanced Stratification:
   EQUAL_34K representation across groups
...
# =============================================================================
# PROPORTIONAL SUBSAMPLING: 18000 CELLS PER CONDITION
# =============================================================================
...
# ========= CONFIG =========
CELLS_PER_CONDITION = 34000     # 34000 cells per condition
TOTAL_TARGET = 68000            # 68000 * 2 conditions
SEED = 42
SAVE_PATH = "./hypoxia_34K_per_condition.dataset"
NUM_PROC = 8
# ==========================

print("="*80)
print("STRATIFIED EQUAL SUBSAMPLING: 34000 CELLS PER CONDITION")
print("="*80)
...
# 1) Sanity checks
for col in ["post_filter_annots", "condition"]:
...
for condition in conditions:
    ...
    df_condition = pd.DataFrame({"post_filter_annots": condition_data["post_filter_annots"], "condition": condition_data["condition"]})
    crosstab = pd.crosstab(df_condition["post_filter_annots"], df_condition["condition"])
...
for condition in conditions:
    ...
    for cell_type, target_count in cell_type_targets.items():
        ...
        cell_type_data = condition_data.filter(
            lambda ex, cls=cell_type: ex["post_filter_annots"] == cls, 
            num_proc=NUM_PROC
        )
...
for condition in conditions:
    ...
    if len(condition_sampled) > 0:
        ...
        df_sampled = pd.DataFrame({"post_filter_annots": condition_sampled["post_filter_annots"], "condition": condition_sampled["condition"]})
        crosstab_sampled = pd.crosstab(df_sampled["post_filter_annots"], df_sampled["condition"])
...
df_sampled = pd.DataFrame({
    "post_filter_annots": ds_sampled["post_filter_annots"], 
    "condition": ds_sampled["condition"]
})

ct_counts = pd.crosstab(df_sampled["post_filter_annots"], df_sampled["condition"]).sort_index()
...
for condition in conditions:
    ...
    orig_df = pd.DataFrame({"post_filter_annots": orig_condition["post_filter_annots"], "condition": orig_condition["condition"]})
    orig_crosstab = pd.crosstab(orig_df["post_filter_annots"], orig_df["condition"])
    ...
    sampled_df = pd.DataFrame({"post_filter_annots": sampled_condition["post_filter_annots"], "condition": sampled_condition["condition"]})
    sampled_crosstab = pd.crosstab(sampled_df["post_filter_annots"], sampled_df["condition"])
...
comparison_df.to_csv("EQUAL_34K_subsampling_comparison.csv", index=False)
print(f"\n✅ Comparison data saved to: EQUAL_subsampling_comparison.csv")
...
# =============================================================================
# BARPLOTS: BEFORE vs AFTER EQUAL_34K SUBSAMPLING
# =============================================================================
...
fig.suptitle('Cell Type Percentages: Before vs After EQUAL_34K Downsampling', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig('EQUAL_34K_subsampling_barplots.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('EQUAL_34K_subsampling_side_by_side.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('EQUAL_34K_subsampling_differences.png', dpi=300, bbox_inches='tight')
plt.show()

print("✅ Barplots saved:")
print("  - EQUAL_34K_subsampling_barplots.png")
print("  - EQUAL_34K_subsampling_side_by_side.png") 
print("  - EQUAL_34K_subsampling_differences.png")
...
# In the currant implementation, the EQUAL_34K sampling does NOT filter cells based on a 1% threshold.
# The resulting folder is : hypoxia_EQUAL_34K_per_condition.dataset
