...
os.chdir("/mnt/nfs/CX000008_DS1/projects/btanasa/hypoxia/test_geneformer_3_full_h5ad.reprocess.sep9/")
...
loom_file = f"{prefix}.adata4_geneformer.loom" if 'prefix' in locals() else "adata4.loom"

# save .loom and include PCA/UMAP/scVI from .obsm

adata4.write_loom(f"{prefix}.adata4_geneformer.loom", write_obsm_varm=True)
print("saved:", f"{prefix}.adata4_geneformer.loom")
...
loom_file = f"{prefix}.adata4_geneformer.loom"  # or set the path directly
...
if 'class' in adata.obs.columns and 'condition' in adata.obs.columns:
    # Use the filtered data from above
    if 'filtered_adata' in locals() and len(filtered_adata) > 0:
        ...
        print("⚠️ Skipping individual plots - no filtered data available")
...
if 'adata4' in locals() and 'class' in adata4.obs.columns:
    ...
    for cell_type in unique_cell_types:
        ...
        filename = f"{prefix}.adata4_geneformer.{cell_type}.h5ad"
...
if 'adata4' in locals() and 'broad_class' in adata4.obs.columns:
    ...
    for broad_class in unique_broad_classes:
        ...
        filename = f"{prefix}.adata4_geneformer.{broad_class}.h5ad"
