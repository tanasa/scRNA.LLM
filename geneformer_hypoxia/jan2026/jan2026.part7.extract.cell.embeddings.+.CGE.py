...
FORWARD_BATCH_SIZE = 24  # Minimum batch size 
NPROC = 24                # Minimum parallelism 
MAX_NCELLS =  2400  
...
filename = f"{Cell_Type}_{datestamp}"
print(filename)

# import os
# base_dir = "/mnt/nfs/CX000008_DS1/projects/btanasa/hypoxia_newobj_jan2026/SCANVI_INTEGRATED_FILTERED.geneformer_compatible.dataset.downsample.18K.model.emb"
...
#######################################################################################################
#######################################################################################################
# extract embedding in CLS model : # Layer 0
#######################################################################################################
####################################################################################################### 
...
try:
    ...
    # filter_data_dict = {"class": ["Imm_CGE"]}
    filter_data_dict = {"class": ["Imm_CGE"]}
    
    # filter_data_dict={"class":["Astroglia","Imm-MGE","Imm-CGE"]} 

    # Setup embedding extractor for cell embeddings
    print(f"🔧 Initializing embedding extractor for cell embeddings...")
    
    # In contrast to Cell embeddings, Gene embeddings are EXTREMELY memory-intensive!
    ...
    embex = EmbExtractor(
        ...
        # Memory optimization: Limit cells if needed 
        ...
        labels_to_plot = ["condition","leiden","orig.ident","class","predictions_scanvi", "donor","post_filter_annots"],
    ...
    # Print structure of cell_embs to understand its format
    ...
    try:
        ...
        if isinstance(cell_embs, pd.DataFrame):
            ...
            if 'embs' in cell_embs:
                ...
                print(f" Warning: Using first key '{first_key}' - verify this is correct!")
            else:
        ...
        if 'df_cell_embs' in locals():
            del df_cell_embs
        if 'embs_data' in locals():
            del embs_data
