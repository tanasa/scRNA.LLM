...
import os, tempfile
# Define your custom tmp path inside /tmp
os.environ["TMPDIR"] = "/tmp/{user}/tmp_local1".format(user=os.getenv("USER"))
# Ensure the directory exists
os.makedirs(os.environ["TMPDIR"], exist_ok=True)
# Tell Python's tempfile module to use it
...
os.chdir("/mnt/nfs/CX000008_DS1/projects/btanasa/hypoxia_newobj_jan2026/SCANVI_INTEGRATED_FILTERED.geneformer_compatible.dataset.downsample.18K.model")
...
CONFIG = {
    ...
    "dataset_path": "/mnt/nfs/CX000008_DS1/projects/btanasa/hypoxia_newobj_jan2026/SCANVI_INTEGRATED_FILTERED.geneformer_compatible.dataset.downsample.18K.dataset",
    "model_path": "/mnt/nfs/CX000008_DS1/projects/btanasa/Geneformer/Geneformer-V2-104M",
    "output_base_dir": "/mnt/nfs/CX000008_DS1/projects/btanasa/hypoxia_newobj_jan2026/SCANVI_INTEGRATED_FILTERED.geneformer_compatible.dataset.downsample.18K.model",
 ...
 #  ^=^n  OPTIMIZED TRAINING PARAMETERS FOR HIGHER ACCURACY
    "num_train_epochs": 0.4,            #  ^|^e decreased from 0.8 to 0.4 for better learning
    # "learning_rate": 0.0001,            #  ^|^e Reduced from 0.0008 for stability
    # "lr_scheduler_type": "cosine",      #  ^|^e Changed from polynomial to cosine (better for longer training)
    "warmup_ratio": 0.1,                #  ^|^e Use ratio instead of absolute steps
    # "weight_decay": 0.01,               #  ^|^e Reduced from 0.03 to 0.01 (less aggressive regularization)
    # "per_device_train_batch_size": 2,   #  ^|^e Reduced from 4 to 2 for stability
    # "per_device_eval_batch_size": 2,    #  ^|^e Match training batch size
    # "forward_batch_size": 2,            #  ^|^e Match batch size
    # "freeze_layers": 1,                 #  ^|^e Reduced from 2 to 1 (less restrictive)
    # "nproc": 16,
    # "ngpu": 1,

    # GPT5 suggestions based on the parameters in Theodoris
    ...
    "per_device_eval_batch_size": 12,      # drop to 6 if eval OOM
    "forward_batch_size": 12,

    "bf16": True,            # L4 supports bfloat16; faster & stable
    "fp16": False,           # prefer bf16 on Ada (L4)
    "gradient_checkpointing": True,  # big saver with long sequences
    "eval_accumulation_steps": 8,    # lowers eval memory peak
    "optimizer": "adamw_torch",

    "freeze_layers": 1,
    "nproc": 16,
    "ngpu": 1,
    "torch_compile": False,  # set True if your stack is PyTorch 2.1+ and stable
...
output_prefix = "hypoxia_18K"  # Updated for optimized condition classification
...
for example in dataset:
    if 'post_filter_annots' in example:
        all_cell_types.add(example['post_filter_annots'])
...
for split_name in ['train', 'eval', 'test']:
    ...
    for example in split_data:
        if 'post_filter_annots' in example:
            split_cell_counts[example['post_filter_annots']] += 1
        elif 'class' in example:
...
for condition_name in all_conditions:
    ...
    for example in condition_data:
        if 'post_filter_annots' in example:
            condition_cell_counts[example['post_filter_annots']] += 1
        elif 'class' in example:
...
for example in dataset:
    cross_tab_data.append({
        ...
        'class': example.get('post_filter_annots', example.get('class', 'unknown'))
    })
...
print(f"\n📈 CELL TYPE vs CONDITION COUNTS:")
celltype_condition_counts = pd.crosstab(df_cross['class'], df_cross['condition'])
print(celltype_condition_counts)

# Cell type vs Condition percentages
print(f"\n📈 CELL TYPE vs CONDITION PERCENTAGES (row-wise):")
celltype_condition_pct = pd.crosstab(df_cross['class'], df_cross['condition'], normalize='index') * 100
    ...
    with open(timing_file, 'w') as f:
        ...
        if training_metrics:
            if 'accuracy' in training_metrics:
                ...
                acc_value = acc[0] if isinstance(acc, list) else acc
                if isinstance(acc_value, (int, float)):
                    f.write(f"Accuracy: {acc_value:.4f}\n")
                else:
                    f.write(f"Accuracy: {acc_value}\n")
            
            if 'macro_f1' in training_metrics:
                f1 = training_metrics['macro_f1']
                f1_value = f1[0] if isinstance(f1, list) else f1
                if isinstance(f1_value, (int, float)):
                    f.write(f"Macro F1-Score: {f1_value:.4f}\n")
                else:
                    f.write(f"Macro F1-Score: {f1_value}\n")
